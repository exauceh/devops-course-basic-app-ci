...
def download_aws_ip_set():
    ...
    with urllib.request.urlopen(aws_ip_range_url,
                                context=ssl_context) as url:
...
def find_ip(ip_range_data, ip_to_find):
    ...
    return [prefix for prefix in ip_range_data
            if ipaddress.ip_address(ip_to_find)
            in ipaddress.ip_network(prefix['ip_prefix'])]
